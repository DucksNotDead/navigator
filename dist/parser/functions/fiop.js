...
const fiopFn = async ({ html, getContent, source, isFillMode }) => {
    const items = [];
    const elements = html
    ...
    for (const element of elements) {
        ...
        for (const key in keys) {
            keys[key] = element.querySelector(selectors[key])?.rawText;
        }
        ...
        const textBlocks = innerContent
            ...
            .map(el => el.rawText);
        let description = '';
        let totalLength = keys.title.length;
        for (const textBlock of textBlocks) {
            totalLength += textBlock.length;
            if (totalLength > const_1.mexMessageContentLength)
                continue;
    ...
    return items.reverse().map(({ title, tag, href, description, imageURL }) => {
        ...
        return {
            ...
            blocks: [
                description,
                (0, utils_1.link)('Посмотреть полностью', href)
            ],
            imagePath: imageURL,
            tags: [tag]
        };
...
exports.fiopFn = fiopFn;
//# sourceMappingURL=fiop.js.map